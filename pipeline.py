#!/usr/bin/env python3
"""
QuarterLevel Lab — deterministic data pipeline (FULL VERSION)
...
Supports CSV:
- With header: columns must include timestamp, open, high, low, close, volume
- Without header: rows like
    2020.01.02,01:00:00,61.391,61.466,61.346,61.406,125
  which maps to:
    date_str,time_str,open,high,low,close,volume
  and timestamp = f"{date_str} {time_str}"

Key feature:
- Store bars_to_* for ALL ladder levels up to K_MAX as LIST columns:
    tp_hit_bars: list[int?]  # index 0 => TP1 (0.125), index 1 => TP2 (0.250), ...
    sl_hit_bars: list[int?]  # same but adverse direction
"""
...
def parse_timestamp_and_date(df: pl.DataFrame) -> pl.DataFrame:
    # Parse timestamp into Datetime and add Date column
    # Works for: "YYYY-MM-DD HH:MM:SS" and "YYYY.MM.DD HH:MM:SS"
...
def write_partitioned_parquet(df: pl.DataFrame, base_dir: Path) -> None:
    """
    Windows-safe writer:
    - Do NOT rely on pyarrow dataset writer to create directory sinks.
    - We manually create partition folders and write actual parquet files.
    Partition: symbol/timeframe/date
    """
    base_dir.mkdir(parents=True, exist_ok=True)
    if base_dir.exists() and not base_dir.is_dir():
        raise RuntimeError(f"Path exists but is not a directory: {base_dir}")

    required = {"symbol", "timeframe", "date"}
    missing = required - set(df.columns)
    if missing:
        raise ValueError(f"Missing partition columns: {missing}")

    # Ensure stable dtypes for partition keys
    df2 = df.with_columns([
    ...
    # Write one file per partition (can be later improved to chunk multiple parts)
    parts = df2.partition_by(["symbol", "timeframe", "date"], as_dict=True)

    for (symbol, timeframe, dt), part in parts.items():
        out_dir = base_dir / f"symbol={symbol}" / f"timeframe={timeframe}" / f"date={dt}"
...
def read_csv_ohlcv(csv_path: Path, has_header: bool = True) -> pl.DataFrame:
    """
    Supported formats:
    ...
    B) Header with date + time:
      date,time,open,high,low,close,volume
      where date can be YYYY.MM.DD or YYYY-MM-DD

    C) No header (7 columns):
      date_str,time_str,open,high,low,close,volume
    """
    if has_header:
        ...
        # Case B: date + time exist
            ...
            df = df.rename({
                ...
                (pl.col("date_str").cast(pl.Utf8) + " " + pl.col("time_str").cast(pl.Utf8)).alias("timestamp")
            ).drop(["date_str", "time_str"])

        else:
            raise ValueError(
                f"Unsupported header columns: {df.columns}. "
                f"Need either (timestamp,open,high,low,close,volume) "
                f"or (date,time,open,high,low,close,volume). "
                f"If your file has no header, use --no-header."
            )

    else:
        # Case C: no header => date_str,time_str,open,high,low,close,volume
        df = pl.read_csv(
            str(csv_path),
            has_header=False,
            new_columns=["date_str", "time_str", "open", "high", "low", "close", "volume"],
...
def detect_level_hits(bars: pl.DataFrame, cfg: LadderConfig) -> pl.DataFrame:
    """
    Detect every bar where [low, high] crosses any fixed 0.25 level.
    Output: (timestamp, date, level, symbol, timeframe)
    """
    hits = (
...
def merge_hits_into_events(hits: pl.DataFrame, cfg: LadderConfig) -> pl.DataFrame:
    """
    Merge hits close in time into one event per level.
    Event start t0 = first hit timestamp of segment.
    """
...
def compute_event_outcomes(bars: pl.DataFrame, events: pl.DataFrame, cfg: LadderConfig) -> pl.DataFrame:
    """
    Fix:
    - Use with_row_index (not with_row_count)
    - Avoid dropping a possibly-nonexistent 'timestamp' after join.
      Instead, join with a renamed key and select exact columns.
    """
    bars = bars.sort("timestamp").with_row_index("bar_index")

    # Create index map with a non-conflicting join key
    idx_map = bars.select([
        pl.col("timestamp").alias("t0"),
        pl.col("bar_index").alias("t0_index"),
    ])

    # Join events to get t0_index
    ev = (
        events
        .join(idx_map, on="t0", how="left")
        .drop_nulls(["t0_index"])
    )

    # only need high/low for ladder logic
    bars_py = bars.select(["high", "low"]).to_dicts()
    ...
    for e in ev.to_dicts():
        ...
        # TP times based on abs excursion max(up, down)
        ...
        # Decide side and compute SL times on adverse side
        if up_max >= down_max:
            side = "UP"
            mfe = up_max
            mae = down_max
        ...
        row = {
            **e,
            ...
            "tp_hit_bars": tp_times,
            "sl_hit_bars": sl_times,
...
def compute_event_features(bars: pl.DataFrame, labeled_events: pl.DataFrame, lookback_bars: int = 20) -> pl.DataFrame:
    ...
    # Pull minimal columns for feature calc
...
def cmd_ingest_raw(args: argparse.Namespace) -> None:
    df = read_csv_ohlcv(Path(args.csv), has_header=not args.no_header)
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p1.add_argument(
        "--no-header",
        action="store_true",
        help="CSV has no header: date,time,open,high,low,close,volume",
    )
    p1.set_defaults(func=cmd_ingest_raw)

    p2 = sub.add_parser("build-events", help="Build labeled events (ALL TP/SL hit bars stored as lists)")
